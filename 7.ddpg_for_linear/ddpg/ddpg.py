...
def learn(save_path, network, env,
        ...
        #   noise_type='normal_0.1',
          noise_type ='ou_1.9',  # large noise to make sure binary action(0,1) can be flipped
    ...
    for epoch in range(nb_epochs):
        ...
        for cycle in range(nb_epoch_cycles):
            ...
            for t_rollout in range(nb_rollout_steps):
                ...
                # counting state pattern of neibour intersection like '12', which indicates greenwave
                for ele in range(len(light_state)):
                    if ele+1<len(light_state):
                        if light_state[ele]==1:
                            if light_state[ele+1]==2:
                                cnt12+=1
                            else:
                                cnt_+=1
                        if light_state[ele]==3:
                            if light_state[ele+1]==1:
                                cnt12+=1
                            else:
                                cnt_+=1                                
                        if light_state[ele]==0:
                            if light_state[ele+1]==3:
                                cnt12+=1
                            else:
                                cnt_+=1
                        if light_state[ele]==2:
                            if light_state[ele+1]==0:
                                cnt12+=1
                            else:
                                cnt_+=1
        ...
        # plt.sca(ax1)
        # # plt.plot(step_set,mean_epoch_episode_rewards)
        # plt.plot(step_set0,average_reward, color='b')
        # # plt.xlabel('Steps')
        # plt.ylabel('Mean Episode Reward')
        ...
        '''4 different plots'''
        # 1. plot some main roads: direction 23, interval 4
        for xx in x:
            for yy in y:
                if xx%2==1:
                    lab=str(xx)+str(yy)
                    # plt.plot(t_set, car_num_set[lab][2],label=lab)
                    plt.plot(t_set, car_num_set[lab][2],label='Q'+str(4*xx-2))
                # lab=str(xx)+str(yy)
                # plt.plot(t_set, car_num_set[lab][2],label=lab)
                # plt.plot(t_set, car_num_set[lab][3],'--',label=i)

        # 2.
        # plt.plot(t_set, car_num_set['11'][2],label='Q'+str(2), color='r')   
        # plt.plot(t_set, car_num_set['31'][2],label='Q'+str(10),color='orange')       
        # plt.plot(t_set, car_num_set['51'][2],label='Q'+str(18),color='g')        
        ...
        print(epoch_state)
        print('12, non 12:', cnt12,cnt_)  # if cnt12 > 1/3 * cnt_, proves green wave preference!
