...
if args.train:
    ...
    # flatten(obs)
    # print('obs',obs[2])
    learning_rate=0.0001  #starts from 0.001
    for steps in range(10000000):
        ...
        reward_set=[[0 for i in range(grid_y+1)]for j in range(grid_x+1)]
        total_reward=0
        for xx in x:
            for yy in y:
                reward_set[xx][yy]=0
                lab=str(xx)+str(yy)
                for i in range (4):
                    total_reward=total_reward - cross[lab].car_nums[i]**2
                    reward_set[xx][yy] = reward_set[xx][yy] - cross[lab].car_nums[i]**3
        ...
        for xx in x:
            for yy in y:
                ...
                RL.store_transition(obs[(xx-1)*grid_y+yy-1],action_set[xx][yy],reward_set[xx][yy],obs_[(xx-1)*grid_y+yy-1])
        if steps%1000000==0:
        ...
        if steps%100==0:
            plt.plot(step_set,total_reward_set)
            plt.savefig('train_re.png')
        if steps%1000==0:
            RL.store()
        total_reward_set.append(total_reward)
...
if args.test:
    ...
    plt.sca(ax1)
    plt.plot(step_set[:plot_len],reward_set[:plot_len],linewidth=2)
    ...
    plt.plot(step_set[:plot_len], Q1_set[:plot_len],'--',label='X1',linewidth=2)
    plt.plot(step_set[:plot_len], Q2_set[:plot_len],label='X2',linewidth=2)
    plt.plot(step_set[:plot_len], Q3_set[:plot_len],label='X3',linewidth=2)
    plt.plot(step_set[:plot_len], Q4_set[:plot_len],label='X4',linewidth=2)
    plt.xlabel('Steps',fontsize=20)
    plt.ylabel('Cars Numbers',fontsize=20)
    plt.tick_params(labelsize=15)
    # set legend
    leg = plt.legend(loc=1,prop={'size': 15})
    legfm = leg.get_frame()
    legfm.set_edgecolor('black') # set legend fame color
    legfm.set_linewidth(0.5)   # set legend fame linewidth

    plt.savefig('test_re.png')
